...
public class Main
{
    ...
    {
        String testFileName = "res/ChatTest.txt";
        List<String> test = Files.readAllLines(
            Paths.get(testFileName));
        ...
        for (String line : test) {
            ...
            for (int i = 2; i < elts.length; i++)
                msgs.add(elts[i] + ", it's me, " + id);
            ChatClient c = new ChatClient(host, port, id);
            ChatClientTask ctask =
                    ChatClientTask.create(c, msgs, wait);
            ctasks.add(ctask);
            es.execute(ctask);
        }
        ctasks.forEach( task -> {
